...
const uint64_t SHA512::K[ROUNDS] = {
...
void SHA512::buffer_to_block(const string& buffer, uint64_t block[])
{
	...
	for (unsigned i = 0; i < BLOCK_LLS; ++i) {
		block[i] = (uint64_t)(buffer[(i << 3) + 7] & 0xff)
			...
			| (uint64_t)(buffer[(i << 3)] & 0xff) << 56;
	}
}

void SHA512::read(std::istream& is, string& s, int max)
{
...
void SHA512::update(const string& s)
{
	istringstream is(s);
	update(is);
}
...
SHA512::SHA512(const string& s) : SHA512()
{
	update(s);
}

void SHA512::transform(uint64_t block[])
{
	...
	for(unsigned i = BLOCK_LLS; i != ROUNDS; ++i)
		W[i] = W[i - 16] + SHA512_F3(W[i - 15]) + W[i - 7] + SHA512_F4(W[i - 2]);

	for (int i = 0; i != ROUNDS; ++i) {
		uint64_t tmp1 = SHA512_F1(state[0]) + SHA2_MAJ(state[0], state[1], state[2]);
		uint64_t tmp2 = K[i] + W[i] + SHA512_F2(state[4]) + SHA2_CH(state[4], state[5], state[6]) + state[7];
