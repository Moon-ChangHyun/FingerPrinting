...
const uint32_t SHA256::K[ROUNDS] = { 
...
void SHA256::buffer_to_block(const string& buffer, uint32_t block[])
{
...
void SHA256::read(std::istream& is, string& s, int max)
{
...
void SHA256::update(const string& s)
{
	istringstream is(s);
	update(is);
}
...
SHA256::SHA256(const string& s) : SHA256()
{
	update(s);
}

void SHA256::transform(uint32_t block[])
{
